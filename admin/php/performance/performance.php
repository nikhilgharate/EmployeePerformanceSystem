	...
	function getRecords($params) {
       $qtot = mysqli_query($this->conn,"SET NAMES utf8"); 
		...
		if( !empty($params['searchPhrase']) ) {   
			...
			$where .=" OR employee_name LIKE '".$params['searchPhrase']."%'";   
			$where .=" OR manager_name LIKE '".$params['searchPhrase']."%'";   

	      $where .=" OR message LIKE '".$params['searchPhrase']."%'";   

			
			$where .=" OR month_name LIKE '".$params['searchPhrase']."%' )";
	   }
	   ...
	   // getting total number records without any search
		$sql = "SELECT * FROM `performance_calculate` ";
		$sqlTot .= $sql;
		$sqlRec .= $sql;
...
function updateEmployee($params) {
	...
	function deleteEmployee($params) {
		...
		$data = array();
		//print_R($_POST);die;
		$sql = "delete from `performance` WHERE ID='".$params["id"]."'";
